...
import { isHidden, pathExists, readDirectory, readFile } from "main/services/fs";
...
export class HeliumWindow {
  ...
  public async openTheme(themePath: string): Promise<OpenThemeResult> {
    ...
    const { theme_author, theme_name, theme_version } = await theme.readThemeInfo(settingsSchemaFilePath);
  ...
  public removeDirectoryWatcher(dirPath: string) {
    if (!this.directoryWatchers.has(dirPath)) throw new Error(`There is no directory path ${dirPath}`)
